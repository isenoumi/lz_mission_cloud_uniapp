    ...
    <z-paging ref="paging" v-model="dataList" @query="queryList" use-page-scroll fixed>
      <view class="content">
        <view class="rankingTitle">
          <view>有限空间名称</view>
          <view style="margin-left: 14rpx">状态</view>
          <view style="width: 120rpx">操作</view>
        </view>
        <u-cell-group>
          <view class="itemBox" v-for="(item, index) in dataList" :key="index">
            <view class="item">
              <view class="itemLeft">
                <view class="title">{{ item.limitedName }}</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      view(item) {
        uni.navigateTo({
          url: '/pageLimited/homeworkLedgerView/info?id=' + item.id,
        })
      },
      edit(item) {
        uni.navigateTo({
