      ...
      <view class="mainBox radius  bg-white">
        <view v-if="hasData">
          <view class="u-flex u-row-between">
            ...
            <u--text type="info" align="right" :text="getType( questioncp[num].answer.choiceType)">
            </u--text>
          ...
          <view class="animation-reverse" :class="animation" data-class="slide-rights"
            ...
            <view class="mentalList" v-for="(item, index) in myList"
              @tap.stop="TogglePanduan(item,index,questioncp[num].answer.choiceType)">
          ...
          <view class="animation-reverse" :class="animation" data-class="slide-rights" v-else>
            <view class="mentalTitle">
              <mp-html :content="questioncp[num].answer.choiceqstContent" />
            </view>
            <view class="mentalList" v-for="(item, index) in questioncp[num].answer.list"
              @tap.stop="Toggle(item,index,questioncp[num].answer.choiceType)">
              <view class="mentalItem" v-bind:class="{ active: item.checked }">
                <view class="indexBox ">
                  <!--            <u--text  align="center" :color="item.checked?'#fff':''" bold
                                        :text="index|answerIndex"></u--text> -->
          ...
          <u-tabbar :fixed="true" :placeholder="true" :safeAreaInsetBottom="true">
            <view class="btns u-flex" :class="{'u-row-right':num!=questioncp.length,'u-row-between':num!=0}">
              ...
              <view class="btn" v-if="(num+1)!=questioncp.length" @click="toNext" :class='{disabled:disabledBtn}'>
                下一题
                <!-- <u-button :disabled="disabledBtn" text="下一题" @click="toNext"></u-button> -->
              </view>
              <view class="btn" v-if="(num+1)==questioncp.length" :class='{disabled:disabledBtn}' @click="toEnd">
                提交
        ...
        <view v-if="noData" class="success">
          <view class="image-box">
            <u--image :showLoading="true"
              src="http://cdn.cestech.com.cn/BkpKnowledge/img/ccf9e86285fe4b52aba247ae4676234a.png" width="140px"
              height="140px">
            </u--image>
          </view>
          <u--text v-if="flg==2" size="15" align="center" bold text="恭喜!答题完成"></u--text>
          <u--text v-else size="15" align="center" bold text="恭喜!全部答对"></u--text>
      ...
      Toggle(info, index, type) {
        ...
        option.forEach((item, index) => {
          if (item.id == info.id) { //如果选中的与数据里面的相等就改变他的样式
            ...
            this.$set(this.questioncp[this.num].answer.list[index], 'checked', !checked)


          } else if (type == 0) { //如果不是多选 只能选中一个 其他的都为false
            this.$set(this.questioncp[this.num].answer.list[index], 'checked', false)
            console.log(222222);
          }
          if (item.checked) {
            answer.push(item.id)
            ...
            uni.showModal({
              ...
              success: function(res) {
                ...
                if (res.confirm) {
                  ...
                  // uni.request({
                  //   url: 'http://192.168.100.61:8089/mission_b/xcx_api/bkbChoiceqst_answer/answerPoints',
                  //   data: { //参数
