...
<template>
	<view class="container">
		<view class="u-nav">
			...
			<navbar :safeAreaInsetTop="true" placeholder height="55" leftIconColor="#fff" autoBack>
				<view class="u-nav-slot" slot="center">
					<u--text text="资料下载" color="#fff"></u--text>
				</view>
		...
		<view v-if="!pageLoading" style="background-color: #FFFFFF;padding: 30rpx 30rpx 30rpx 30rpx;">
			<view class="titleBox text-xl text-black text-bold">{{detail.title||''}}
			</view>
			<view class="u-m-t u-m-b">
				<u--text type="info" size="13" :text="detail.create_time">
				</u--text>
			</view>

			<u-line></u-line>
			<view style="margin-top: 30rpx;" class="titleBox text-xl text-black text-bold">{{detail.title+'相关资料如下'||''}}
			</view>

			<view class="u-m-t u-m-b" v-for="item in list">
				<video v-if="item.name && item.name.endsWith('mp4')" :src="item.value" muted autoplay loop preload x5-video-player-fullscreen="true" x5-playsinline
					playsinline webkit-playsinline></video>
				<u--text  v-if="item.name && item.name.endsWith('mp4')" @click="uploadVideo(item.value)" type="primary" size="18" :text="item.name">
				</u--text>
				<u--text v-else @click="downloadFun(item.value,item.name)" type="primary" size="18" :text="item.name">
				</u--text>
			</view>
			<!-- <view v-for="item in list">
				<video :src="item.value" muted autoplay loop preload x5-video-player-fullscreen="true" x5-playsinline
					playsinline webkit-playsinline></video>
	...
	export default {
		...
		methods: {
			...
			downloadFun(enclosure, name) {
				...
				if (enclosure.indexOf("jpg") != -1) {
					txt = 'jpg'
				}
				if (enclosure.indexOf("mp4") != -1) {
					txt = 'mp4'
				}
