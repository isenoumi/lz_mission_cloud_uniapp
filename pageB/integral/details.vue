	...
	<view class="conetent">
		...
		<view class="order-number">
			...
			<view v-if="details.order.status==0"
				...
				class="status">
				待发货</view>
			<view v-if="details.order.status==1" style="color:#39d48f ;" class="status">已发货</view>
			<view v-if="details.order.status==2" style="color: silver;" class="status">已完成</view>
		</view>
		<view class="shop-details">
			<view>
				<image :src="details.orderItem.pic" style="width: 140rpx;height: 140rpx;" mode="scaleToFill">
				</image>
			</view>
			<view style="margin-left: 20rpx;">
				<view class="name">{{details.orderItem.prodName}}</view>
				<view class="specifications">{{details.orderItem.skuName}}*{{details.order.productNums}}</view>
			</view>
		</view>
		...
		<view class="item" v-if="details.order.status!=0">物流公司：{{details.name}}</view>
		<view class="item" v-if="details.order.status!=0">物流单号：{{details.order.dvyFlowId}}</view>
